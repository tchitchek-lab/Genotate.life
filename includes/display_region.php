...
<link rel="stylesheet" media="screen" type="text/css" href="../css/bootstrap-theme.min.css">
...
<link rel="stylesheet" media="screen" type="text/css" href="../css/div.css">
<link rel="stylesheet" media="screen" type="text/css" href="../css/element.css">
<link rel="stylesheet" media="screen" type="text/css" href="../css/input.css">
...
<link rel="stylesheet" media="screen" type="text/css" href="../css/range.css">
...
$transcript_seq = get_transcript ( 0, $dataset_id, $transcript_name, $temp_transcript_id );
$region_seq = get_region ( 0, $dataset_id, $transcript_name, $temp_region_id );
if ($region_coding != "noncoding") {
	$prot_seq = get_protein ( 0, $dataset_id, $transcript_name, $temp_region_id );
}
...
if ($region_coding == "coding") {
	echo "$region_type ORF of " . number_format ( $region_size, 0, '.', ',' ) . " bases on transcript ".substr($transcript_name,0,15)." ({$region_strand}) ";
} else {
	echo "ncRNA of " . number_format ( $region_size, 0, '.', ',' ) . " bases on transcript ".substr($transcript_name,0,15)." ({$region_strand})";
}
...
//BELOW SVG
echo "<form name='transcript' id='transcript_$region_id' action='./{$path_includes}download.php?type=transcript' method='post' target=\"_blank\">";
...
echo "<input type='hidden' name='temp_transcript_id' value=$temp_transcript_id></form>";

echo "<form name='region' id='region_$region_id' action='./{$path_includes}download.php?type=region' method='post' target=\"_blank\">";
echo "<input type='hidden' name='dataset_id' value=$dataset_id>";
echo "<input type='hidden' name='dataset_name' value=$dataset_name>";
echo "<input type='hidden' name='transcript_name' value=$transcript_name>";
echo "<input type='hidden' name='temp_region_id' value=$temp_region_id></form>";
if ($region_coding != "noncoding") {
	echo "<form name='protein' id='protein_$region_id' action='./{$path_includes}download.php?type=protein' method='post' target=\"_blank\">";
	echo "<input type='hidden' name='dataset_id' value=$dataset_id>";
	echo "<input type='hidden' name='dataset_name' value=$dataset_name>";
	echo "<input type='hidden' name='transcript_name' value=$transcript_name>";
	echo "<input type='hidden' name='temp_region_id' value=$temp_region_id></form>";
}
echo "<form name='annotation' id='annotation_$region_id' action='./{$path_includes}download.php?type=annotation' method='post' target=\"_blank\">";
echo "<input type='hidden' name='dataset_id' value=$dataset_id>";
echo "<input type='hidden' name='dataset_name' value=$dataset_name>";
echo "<input type='hidden' name='transcript_name' value=$transcript_name>";
echo "<input type='hidden' name='region_id' value=$region_id></form>";
echo "<form name='annotation' id='BLASTN_$region_id' action='https://blast.ncbi.nlm.nih.gov/Blast.cgi?PAGE_TYPE=BlastSearch' method='post' target=\"_blank\">";
...
<div class="dropdown" style='float: right;'>
	<button class="btn btn-xs btn-default dropdown-toggle" type="button" data-toggle="dropdown" style='width: 11em;'>
	...
	if ($region_coding == "coding") {
		...
		echo "<li><a href=\"javascript:document.getElementById('region_$region_id').submit();\">nucleic sequence</a></li>";
...
<div class="dropdown" style='float: right;'>
	...
	<ul class="dropdown-menu">
			<?php
	echo "<li><a href=\"javascript:document.getElementById('BLASTN_$region_id').submit();\">nucleic sequence</a></li>";
	if ($region_coding != "noncoding") {
