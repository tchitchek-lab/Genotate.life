...
<div style='width: 420px; padding: 0; margin: 0; padding-bottom: 5px; height: 300px;'>
	<div class="div-border-title">
		ORF identification parameters <a style='float: right; margin-right: 10px;' data-toggle="tooltip" data-placement="top" href="./index.php?page=tutorial" target="_blank" title="<?php echo $tooltip_text['orf_panel'];?>"> <img src="/img/tutorial.svg" style='margin-bottom: 2px; height: 20px; filter: invert(90%);'></a>
	</div>
	<div class="div-border" style='padding: 5px; height: 260px;'>
		<div style="width: 100%;">
			<label for="start_codon">start codon(s)</label>
				<div style="float:right;">
					<button data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['code_standard'];?>" class='btn btn-sm btn-secondary' style='border:1px solid lightgrey;margin-bottom:2px;' onclick="$('#start_codon').val('ATG');$('#stop_codon').val('TAG,TGA,TAA');" form=''>Standard</button>
					<button data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['code_vert_mito'];?>" class='btn btn-sm btn-secondary' style='border:1px solid lightgrey;margin-bottom:2px;' onclick="$('#start_codon').val('ATT,ATC,ATA,ATG,GTG');$('#stop_codon').val('TAG,AGA,TAA,AGG');" form=''>Vertebrate Mito.</button>
					<button data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['code_invert_mito'];?>" class='btn btn-sm btn-secondary' style='border:1px solid lightgrey;margin-bottom:2px;' onclick="$('#start_codon').val('ATG,ATA,TTG,ATT');$('#stop_codon').val('TAG,TAA');" form=''>Invertebrate Mito.</button>
				</div>
			<input data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['start_codon'];?>" type="text" id="start_codon" name="start_codon" value="ATG" onchange='check_start_codon()' onkeyup='check_start_codon()' style="width: 100%; height: 2em; text-align: left;">
		</div>
		<div style="width: 100%;">
			<label for="stop_codon">stop codon(s)</label>
			<input data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['stop_codon'];?>" type="text" id="stop_codon" name="stop_codon" value="TAG,TGA,TAA" onchange='check_stop_codon()' onkeyup='check_stop_codon()' style="width: 100%; height: 2em; text-align: left;">
		</div>
		<p>&nbsp;</p>
		<div data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['orf_min_size'];?>" style="width: 100%;">
			<div class="input-group" style="padding: 0; margin: 0; margin-top: 5px; width: 50%; height: 30px;">
			...
			<div class="input-group" style="padding: 0; margin: 0; margin-top: 5px; width: 50%; height: 30px;">
				<input class="form-control" type="number" id="orf_min_size" name="orf_min_size" min="6" max="9999999" value="300" style="width: 100%; height: 30px; text-align: right; float: right">
				<span class='input-group-addon' style='padding-top: 0; padding-bottom: 0;'>bases</span>
			</div>
		</div>
		<div style="width: 100%; padding: 0; margin: 0; margin-top: 5px; margin-bottom: 5px;">
			<span data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['inner'];?>" style="width: 50%; float: left;"> <label data-toggle='buttons' class='btn btn-default' for="inner_orf" style="width: 100%;">
					<input type="checkbox" id="inner_orf" name="inner_orf" style='display: none;'>
					compute inner ORFs
				</label>
			</span> <span data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['outside'];?>" style="width: 50%; float: left;"> <label data-toggle='buttons' class='btn btn-default' for="outside_orf" style="width: 100%;">
					<input type="checkbox" id="outside_orf" name="outside_orf" style='display: none;'>
					compute outside ORFs
				</label>
			</span>
		</div>
		<div style="width: 100%; padding: 0; margin: 0; margin-bottom: 5px;">
			<span data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['compute_ncRNA'];?>" style="width: 50%; float: left;"> <label data-toggle='buttons' class='btn btn-default active' for="ncRNA" style="width: 100%;">
					<input type="checkbox" id="compute_ncrna" name="compute_ncrna" checked style='display: none;'>
					compute ncRNA
				</label>
			</span> <span data-toggle="tooltip" data-placement="top" title="<?php echo $tooltip_text['compute_both_strands'];?>" style="width: 50%; float: left;"> <label data-toggle='buttons' class='btn btn-default active' for="compute_reverse" style="width: 100%;">
...
<div style='width: 420px; padding: 0; margin: 0; padding-right:5px;padding-bottom: 5px; height: 310px;'>
	<div class="div-border-title">
	...
	<div class="div-border" style='padding: 5px; height: 270px;'>
		<div style='width: 100%; height: 260px; overflow: auto; overflow-y: scroll;'>
<?php
$request = "SELECT * FROM blast WHERE state = 'complete' ORDER BY name";
$results = mysqli_query ( $connexion, $request ) or die ( "SQL Error:<br>$request<br>" . mysqli_error ( $connexion ) );
while ( $row = mysqli_fetch_array ( $results, MYSQLI_ASSOC ) ) {
	$id = $row ['blast_id'];
	$key = $row ['name'];
	echo "<div style='align:left;padding:2px;width:100%;'>";
	...
	if ($row ['type'] == 'nucleic') {
		echo "<input type='checkbox' name='BLASTN[]' id='$id' value='$id' unchecked style='display:none;'>" . substr ( $key, 0, 40 ) . "</label>";
	} else {
		echo "<input type='checkbox' name='BLASTP[]' id='$id' value='$id' unchecked style='display:none;'>" . substr ( $key, 0, 40 ) . "</label>";
	}
...
<div style='width: 420px; padding: 0; margin: 0; padding-bottom: 5px; height: 310px;'>
	<div class="div-border-title">
	...
	<div class="div-border" style='padding: 5px; height: 270px;'>
		<div id='functional_services_container' style='width: 100%; height: 260px; overflow: auto; overflow-y: scroll;'>
			<?php
			include ("services_info.php");
			foreach ( $services_info as $service => $info_service ) {
				if (substr_count ( $service, "BLAST" ) == 0) {
					$key = $service;
					...
					if (substr_count ( $service, "SABLE" ) == 0) {
						echo "<label data-toggle='buttons' id='$key' class='btn btn-sm btn-default func_annot' for='$key' style='width:90%;'>";
						echo "<input type=\"checkbox\" name='SERVICE[]' id='$key' value='$key' class='func_annot_checkbox' unchecked style='display:none;'>$key</label>";
					}else{
						echo "<label data-toggle='buttons' id='$key' class='btn btn-sm btn-default' for='$key' style='width:90%;'>";
						echo "<input type=\"checkbox\" name='SERVICE[]' id='$key' value='$key' unchecked style='display:none;'>$key</label>";
					}
					...
					echo "</div>";
          if (substr_count ( $service, "SIGNALP" ) != 0) {
            echo "<div class='collapse' id='collapse_$key' style='width:100%;'>";
            echo "<div class='card card-block' style='width:100%;margin:0px;'>";
            ...
            echo "<input class='form-control' step='0.01' type='number' id='score_$key' name='score_$key' min='$score_min' max='$score_max' value='$value' style='width:100%;height:24px;text-align: right;'>";
            echo "</div>";
            ?>
            <div class="btn-group" data-toggle="buttons" style="width: 100%; margin-bottom: 5px;">
              <label class='btn btn-default active' style="width: 50%">
                eukaryote
                <input id='eukaryote' value='eukaryote' type='radio' name='kingdom' style='display: none' checked>
              </label>
              <label class='btn btn-default' style="width: 50%;">
                prokaryote
                <input id='prokaryote' value='prokaryote' type='radio' name='kingdom' style='display: none' unchecked>
              </label>
            </div>
            <?php
            echo "</div>";
