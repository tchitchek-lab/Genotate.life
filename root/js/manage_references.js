function rename_reference_dataset(analysis_id) {
    ...
    if (new_name !== "" && new_name) {
        ...
        xhr.open('GET', '/root/includes/update_references.php?action=rename&analysis_id=' + analysis_id + '&new_name=' + new_name);
...
function delete_reference_dataset(analysis_id) {
    if (confirm('Are you sure to delete this dataset?')) {
        const xhr = new XMLHttpRequest();
        xhr.open('GET', '/root/includes/update_references.php?action=delete&analysis_id=' + analysis_id);
        xhr.addEventListener('readystatechange', function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                location.reload();
            }
