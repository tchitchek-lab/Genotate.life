function rename_annotation_dataset(analysis_id) {
    ...
    if (new_name !== "" && new_name) {
        ...
        xhr.open('GET', '/admin/includes/update_annotations.php?action=rename&analysis_id=' + analysis_id + '&new_name=' + new_name);
...
function delete_annotation_dataset(analysis_id) {
    if (confirm('Are you sure to delete this dataset?')) {
        const xhr = new XMLHttpRequest();
        xhr.open('GET', '/admin/includes/update_annotations.php?action=delete&analysis_id=' + analysis_id);
        xhr.addEventListener('readystatechange', function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                location.reload();
            }
...
function interrupt_annotation_dataset(analysis_id) {
    if (confirm('Do you want to interrupt this dataset ?')) {
        const xhr = new XMLHttpRequest();
        xhr.open('GET', '/admin/includes/update_annotations.php?action=interrupt&analysis_id=' + analysis_id);
        xhr.addEventListener('readystatechange', function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                location.reload();
            }
