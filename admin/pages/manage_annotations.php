...
<script src="/admin/js/manage_annotations.js?v=618546156" xmlns:right></script>
...
<div class="div-border">
    ...
    <table class='manage_tables' style='width: 100%;'>
        ...
        echo "<form name='transcript_$analysis_id' id='transcript_$analysis_id' action='/admin/includes/download_files.php' method='post' target=\"_blank\">";
        ...
        echo "<form name='region_$analysis_id' id='region_$analysis_id' action='/admin/includes/download_files.php' method='post' target=\"_blank\">";
        echo "<input type='hidden' name='analysis_id' value='/$analysis_id/$file_name'>";
        echo "<input type='hidden' name='type' value='annotation'></form>";
        $file_name = "regions_prot.fasta";
        echo "<form name='protein_$analysis_id' id='protein_$analysis_id' action='/admin/includes/download_files.php' method='post' target=\"_blank\">";
        echo "<input type='hidden' name='analysis_id' value='/$analysis_id/$file_name'>";
        echo "<input type='hidden' name='type' value='annotation'></form>";
        $file_name = "all_annotations.txt";
        echo "<form name='annotation_$analysis_id' id='annotation_$analysis_id' action='/admin/includes/download_files.php' method='post' target=\"_blank\">";
        echo "<input type='hidden' name='analysis_id' value='/$analysis_id/$file_name'>";
        echo "<input type='hidden' name='type' value='annotation'></form>";
        echo "<tr>";
        ...
        echo "<td>";
        echo "<a href='/admin/index.php?page=manage_annotation_details&encoded_id=$encoded_id'><button style='width:30px;height:30px;padding:5px;' data-toggle='tooltip' data-placement='top' title='".$tooltip_text['manage_getinfo']."' class='btn btn-md btn-primary'>";
        echo "<span class='glyphicon glyphicon-question-sign' aria-hidden='true'></span></button></a>";

        if ($row ['status'] != "computing") {
            echo "<a href='/admin/index.php?page=view_annotations&encoded_id=$encoded_id'><button style='width:30px;height:30px;padding:5px;' data-toggle='tooltip' data-placement='top' title='view annotation results' class='btn btn-md btn-primary' >";
            echo "<span class='glyphicon glyphicon-th-list' aria-hidden='true'></span></button></a>";
        }else{
            echo "<a href='/admin/index.php?page=view_annotations&encoded_id=$encoded_id'><button style='width:30px;height:30px;padding:5px;' data-toggle='tooltip' data-placement='top' title='view annotation results' class='btn btn-md btn-primary' disabled>";
            echo "<span class='glyphicon glyphicon-th-list' aria-hidden='true'></span></button></a>";
        }
        if ($row ['status'] != "computing") {
